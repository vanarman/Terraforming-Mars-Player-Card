...
const StandardProjectModal = () => {
  const [selectedAction, setSelectedAction] = useState<StandardProjectType | null>(null);
  const [patentsAmount, setPatentsAmount] = useState<number>(0);
  const modalVisibility = useSelector((state: RootState) => state.modal["standardProjectModalVisibility"])
  ...
  const imageSrc = () => {
    if (selectedAction === StandardProjectType.ASTEROID) {
      return require('../../assets/tilePlacement/temperatureRise.gif');
    }
    if (selectedAction === StandardProjectType.AQUIFER) {
      return require('../../assets/tilePlacement/waterTilePlacement.gif');
    }
    if (selectedAction === StandardProjectType.GREENERY) {
      return require('../../assets/tilePlacement/greeneryTilePlacement.gif');
    }
    if (selectedAction === StandardProjectType.CITY) {
      return require('../../assets/tilePlacement/cityTilePlacement.gif');
    }
  ...
  return (
    <>
      <Modal
        ...
        onClose={() => dispatch(hide({ modalType: "standardProjectModalVisibility" }))}
      >
        <ModalBackdrop />
        <ModalContent>
          <ModalHeader>
            <Heading size="md">{t('modal.standardProject.title')}</Heading>
            <ModalCloseButton onPress={() => dispatch(hide({ modalType: "standardProjectModalVisibility" }))}>
              <Icon name="SquareX" />
            </ModalCloseButton>
          </ModalHeader>
            ...
            <Box justifyContent="space-between" alignItems="center" my="$2" px="$2" flexDirection="row">
              ...
              <Box flexDirection="row" w="$1/2">
                ...
                <Button w="$1/2" isDisabled={credits.current < 11} onPress={() => {
                  ...
                  <ButtonText>{t('modal.standardProject.actions.patents.button')}</ButtonText>
                </Button>
              </Box>
            </Box>
            <Box justifyContent="space-between" alignItems="center" my="$2" px="$2" flexDirection="row">
              ...
              <Button w="$1/2" isDisabled={credits.current < 11} onPress={() => {
                dispatch(standardProject({ type: StandardProjectType.POWER_PLANT }));
                dispatch(hide({ modalType: "standardProjectModalVisibility" }));
              }}>
                <ButtonText>{t('modal.standardProject.actions.powerPlant.button')}</ButtonText>
              </Button>
            </Box>
            <Box justifyContent="space-between" alignItems="center" my="$2" px="$2" flexDirection="row">
              <Text>{t('modal.standardProject.actions.asteroid.title')}</Text>
              <Button w="$1/2" isDisabled={credits.current < 14} onPress={() => {
                setSelectedAction(StandardProjectType.ASTEROID);
              }}>
                <ButtonText>{t('modal.standardProject.actions.asteroid.button')}</ButtonText>
              </Button>
            </Box>
            <Box justifyContent="space-between" alignItems="center" my="$2" px="$2" flexDirection="row">
              <Text>{t('modal.standardProject.actions.aquifer.title')}</Text>
              <Button w="$1/2" isDisabled={credits.current < 18} onPress={() => {
                setSelectedAction(StandardProjectType.AQUIFER);
              }}>
                <ButtonText>{t('modal.standardProject.actions.aquifer.button')}</ButtonText>
              </Button>
            </Box>
            <Box justifyContent="space-between" alignItems="center" my="$2" px="$2" flexDirection="row">
              <Text>{t('modal.standardProject.actions.greenery.title')}</Text>
              <Button w="$1/2" isDisabled={credits.current < 23} onPress={() => {
                setSelectedAction(StandardProjectType.GREENERY);
              }}>
                <ButtonText>{t('modal.standardProject.actions.greenery.button')}</ButtonText>
              </Button>
            </Box>
            <Box justifyContent="space-between" alignItems="center" my="$2" px="$2" flexDirection="row">
              <Text>{t('modal.standardProject.actions.city.title')}</Text>
              <Button w="$1/2" isDisabled={credits.current < 25} onPress={() => {
                setSelectedAction(StandardProjectType.CITY);
              }}>
                <ButtonText>{t('modal.standardProject.actions.city.button')}</ButtonText>
              </Button>
            </Box>
          </ModalBody>
        </ModalContent>
      ...
      <AlertDialog isOpen={selectedAction != null}>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            <Text size="lg">
              {t(`modal.standardProject.tilePlacement.${selectedAction?.toString()}`)}
            </Text>
            <Image size="xl" m="$3" alignSelf='center' source={imageSrc()} alt={`Placement of ${selectedAction?.toString()} tile`}  />
          </AlertDialogBody>
          <AlertDialogFooter>
